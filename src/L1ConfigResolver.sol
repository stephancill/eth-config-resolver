...
contract L1ConfigResolver is GatewayFetchTarget, IERC165, IExtendedResolver {
    ...
    // ============ Constructor ============
    constructor(IGatewayVerifier _verifier, address _l2ConfigResolver) {
        verifier = _verifier;
    ...
    function supportsInterface(bytes4 interfaceId) external pure override returns (bool) {
        return interfaceId == type(IERC165).interfaceId || interfaceId == type(IExtendedResolver).interfaceId
            || interfaceId == type(IAddrResolver).interfaceId || interfaceId == type(IAddressResolver).interfaceId
    ...
    function resolve(bytes calldata, bytes calldata data) external view override returns (bytes memory) {
        ...
        bytes32 node;
        if (selector == IAddrResolver.addr.selector || selector == IContentHashResolver.contenthash.selector) {
        ...
        fetch(verifier, req, this.resolveCallback.selector, data, new string[](0));
    }
    ...
    /// @return The ABI-encoded result
    function resolveCallback(bytes[] calldata values, uint8 exitCode, bytes calldata data)
        external
        pure
    ...
    function addr(bytes32 node) external view returns (address) {
        ...
        fetch(verifier, req, this.addrCallback.selector, abi.encode(node), new string[](0));
    }

    /// @notice Callback for addr()
    function addrCallback(bytes[] calldata values, uint8 exitCode, bytes calldata) external pure returns (address) {
    ...
    function textCallback(bytes[] calldata values, uint8 exitCode, bytes calldata)
        external
        pure
    ...
    function contenthash(bytes32 node) external view returns (bytes memory) {
        ...
        fetch(verifier, req, this.contenthashCallback.selector, "", new string[](0));
    }

    /// @notice Callback for contenthash()
    function contenthashCallback(bytes[] calldata values, uint8 exitCode, bytes calldata)
        external
        pure
        returns (bytes memory)
    {
    ...
    function _buildRequest(bytes32 node, bytes calldata data) internal view returns (GatewayRequest memory) {
        bytes4 selector = bytes4(data);
    ...
    function _emptyResponse(bytes4 selector) internal pure returns (bytes memory) {
        if (selector == IAddrResolver.addr.selector) {
            return abi.encode(address(0));
        } else if (selector == IAddressResolver.addr.selector) {
